...
def downloadImg(folderPath, imgUrl, index):
    ...
    try:
        ...
        if printLogEnabled:
            print ('正在下载第' + str(index + 1) + '张图片，图片地址:' + str(imgUrl))
        # ------ IO处理 ------
        ...
        if len(imgeNameFromUrl) < 8:
            imgName = random_str(4) + random_str(1,
                                                 '123456789') + random_str(2,
                                                                           '0123456789') + "_" + imgeNameFromUrl
...
def downloadImgList(folderPath, imgList):
    ...
    if not poolSupport:
        ...
        t.join()  # 父线程，等待所有线程结束
        if len(imgList) > 0:
            print ('下载结束，存放图片目录：' + str(folderPath))
    else:
        # print ('线程池模式')
        # ------ 线程池编程 ------
        futures = []
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=thePoolSize) as pool:
            ...
            for f in result.done:
                if f.result():
                    suc += 1
            print('下载结束，总数：' + str(len(imgList)) + '，成功数：' +
                  str(suc) + '，存放图片目录：' + str(folderPath))


# ------ 下载百度帖子内所有图片 ------
# folderPath 定义图片存放的目录 url 百度贴吧链接
def downloadImgFromBaidutieba(folderPath='tieba',
                              url='https://tieba.baidu.com/p/5256331871'):
    html = getHtml(url)
    # ------ 利用正则表达式匹配网页内容找到图片地址 ------
    # reg = r'src="(.*?\.jpg)"'
    reg = r'src="(.*?/sign=.*?\.jpg)"'
...
if __name__ == '__main__':
    ...
    # 下载百度帖子内所有图片
    downloadImgFromBaidutieba(
        'tieba\\' + now,
        'https://tieba.baidu.com/p/5256331871')
    #  爬虫搜索并下载美女图片地址：http://huaban.com
